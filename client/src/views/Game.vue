...
watch(
  () => route.params.id,
  (newId) => (id.value = parseInt(newId.toString()))
);
const game = computed(() => ongoingGameStore.game(id.value));
const enabled = computed(
  () =>
    game.value !== undefined &&
    playerStore.player === game.value.players[game.value.playerInTurn]
);
const finished = computed(
  () => game.value === undefined || is_finished(game.value)
);
const standings = computed(() => {
  if (game.value === undefined) return [];
  const g = game.value;
  const standings: [string, number][] = scores(g).map((s, i) => [
    g.players[i],
    s,
  ]);
...
if (playerStore.player === undefined) router.push(`/login?game=${id.value}`);
      ...
      <table>
        ...
        <tbody>
          <tr
            ...
            :class="row[0] == playerStore.player ? 'current' : undefined"
          >
            <td>{{ row[0] }}</td>
            <td>{{ row[1] }}</td>
          </tr>
