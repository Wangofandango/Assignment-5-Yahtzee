...
import {
  ...
  small_straight_slot,
  trips_slot,
  two_pair_slot as two_pairs_slot,
  yahtzee_slot,
  type Slot,
} from "./yahtzee.slots";
...
export function register_upper(
  ...
  const scores = {
    ...section.scores,
    [value]: score(upper_section_slots[value], roll),
  };
...
export const lower_section_keys: Readonly<LowerSectionKey[]> = Object.keys(
  lower_section_slots
) as LowerSectionKey[];
...
export function register_lower(
  ...
  const scores = {
    ...section.scores,
    [key]: score(lower_section_slots[key], roll),
  };
  return { scores };
}
