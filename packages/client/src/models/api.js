...
export async function pendingGames(): Promise<IndexedYahtzeeSpecs[]> {
...
export async function newGame(numberOfPlayers: number, player: string): Promise<IndexedYahtzeeSpecs | IndexedYahtzee> {
  return await post('http://localhost:8080/pending-games', { creator: player, number_of_players: numberOfPlayers });
}

async function performAction(game: IndexedYahtzee, action: any) {
...
export async function reroll(game: IndexedYahtzee, held: number[], player: string) {
  return performAction(game, { type: 'reroll', held, player });
}

export async function register(game: IndexedYahtzee, slot: DieValue | LowerSectionKey, player: string) {
  return performAction(game, { type: 'register', slot, player });
}